...
const STATUS_OPTIONS = [
  ...
  {
    value:
      `${PAYMENT_FILTER_PREFIX}paid`,
    label: "Paid",
  },
  {
    value:
      `${PAYMENT_FILTER_PREFIX}partially_paid`,
...
export default function Bookings() {
  ...
  /*
   * This single state controls the existing
   * combobox.
   *
   * Normal values represent booking statuses.
   * Values prefixed with "payment:" represent
  ...
  useEffect(() => {
    ...
    const loadBookings =
      async () => {
        ...
        /*
         * The administrator still sees one
         * combobox, but booking status and payment
         * status remain separate API parameters.
         */
        const isPaymentFilter =
          status.startsWith(
    ...
    <PageContainer>
      ...
      <div className="mt-8 rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row">
          ...
          {/*
           * This remains the only filtering
           * combobox on the page.
           */}
          <select
            value={status}
            aria-label="Booking or payment status"
            onChange={(event) => {
              setStatus(
                event.target.value,
              );
